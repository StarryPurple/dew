...
```dew
struct Arr(T, N: Int) { data: [T; N] }
// T: type, N: Int compile-time constant

def identity = fn(a: [Int; N]) -> [Int; N] { a }
...
**No `Float` in initial version.** String literals are `[Char; N]` — no `String` type name.
...
struct Matrix(T, R: Int, C: Int) {
  data: [T; R * C],   // future: const arithmetic
}
...
enum Node {
  Text([Char; 8]),
  Loaded { data: [Char; 8], ref: Affine(Int) },
}
```
...
```dew
def arr = [1, 2, 3]           // [Int; 3] — N inferred from element count
def arr: [Int; 3]        // explicit N as literal
```

**Fill syntax.** `[expr; N]` constructs an array where every element is `expr` — no need to write `N` copies:

```dew
def arr = [0; 100];           // [Int; 100], all zeros
def arr = [true; 10];         // [Bool; 10], all true
def arr: [Int; 1000] = [10; 1000];  // all tens
```
...
```dew
def identity = fn(a: [Int; N]) -> [Int; N] { a }
def duplicate = fn(a: [T; N]) -> Array(T, N + N) { ... }  // future: arithmetic in const generics
```

String literals are arrays of Char: `"hello"` is `[Char; 5]`.
...
((Int, Int) -> Char, Int) -> [Char; 10]   // HOF: takes a function param
```
...
def map_char = fn(f: (Int) -> Char, xs: [Int; 10]) -> [Char; 10] {
  // ...
}
// map_char: ((Int) -> Char, [Int; 10]) -> [Char; 10]
```
...
type_match typeof(x) {
  ...
  [Char; _] => x -> stdout,  // x narrows to Array(Char, N) — N is wild
  _        => Unit,           // fallback
}
```
...
**Const generic binding.** When a `type_match` arm matches a type with const generic parameters, the parameters are bound to their concrete values and usable as compile-time constants in the arm body:

```dew
def array_size = fn(x: T) -> Int {
  type_match typeof(x) {
    [_; N] => N,      // N bound to concrete size, usable as Int
    _           => -1,
  }
}

def arr = [1, 2, 3, 4, 5];
array_size(arr);             // → 5 — N = 5, extracted at compile time
```

This works because monomorphization resolves all const generics before code generation — `N` is a known literal, not a runtime variable. The compiler substitutes `5` directly into the generated code.

> **Not usable in function signatures.** `fn(x: [T; N]) -> N` would require dependent types (the return type depends on a value parameter). Const generic binding works only within `type_match` arm bodies where `N` is already resolved. Writing `-> N` in a function signature is rejected — use `-> Int` instead.
...
def greet = fn(name: [Char; 32]) { name -> stdout; }
def greet = fn(name: [Char; 32]) -> Unit { name -> stdout; Unit }
```
...
// Surface: (&Point, Int, &Int, [Char; 8]) -> Point
def process = fn(&p: Point, scale: Int, &color: Int, text: [Char; 8]) -> Point {
  &p { x = p.x * scale };
  &color = color + 1;
  p
}
...
def process = fn(p: Point, scale: Int, color: Int, text: [Char; 8])
               -> (Point, Int, Point) {
  def p = p { x = p.x * scale };
  def color = color + 1;
  (p, color, p)
...
| [T; N] | Array of N default T values |
...
| Cast (postfix) | `as` | Int, Bool, Char | `(x as Int)`, `(flag as Bool)` |
...
**Cast expression (`expr as Type`).** Converts between primitive types at the expression level. The target type must be `Int`, `Bool`, or `Char` — `Unit` is forbidden on both sides. The expression is a postfix operator binding at precedence level 1 (tightest, same as `.` field access and `()` call):

```dew
def x = 42;
def b = (x as Bool);     // Int → Bool (nonzero becomes true)
def y = (b as Int);      // Bool → Int (true becomes 1, false becomes 0)
def c = (42 as Char);    // Int → Char (by codepoint)
```

Cast is a type-level annotation that reinterprets the bit pattern at compile time. At the IR level, the value passes through unchanged — the type information is erased. The type checker validates both the source and target types are primitive and permits type variables (unresolved types) as source when the cast target is a valid primitive.

> **Cast is added for Rx→Dew interop.** Rx uses `expr as i32` / `expr as usize` as no-op casts in boolean-to-integer conversions. Without the cast expression, the translator would drop these casts and produce type errors when `Bool` values appear in `Int` contexts.
...
|------------|-----------|
| 1 (tightest) | `.` field access, `[]` subscript, `()` call, `as` cast |
...
> `Unit` carries no information. Printing it is a compile error — there is no meaningful text representation. `"Unit"` ([Char; 4]) is not `Unit`; inventing `()` or `#Unit` output would introduce a concept with no counterpart in Dew source. Functions returning `Unit` are called for their side effects; their result is never printed.
...
def ys = Cons("a", Cons("b", Nil));         // List([Char; 1])
```
...
- `array_to_list: [T; N] -> List(T)` — converts a fixed-size array to a linked list. `N` is known at compile time from the array type.
- `list_to_array` — **not provided**. Array size `N` is part of the type and must be a literal; a list carries no size information. Converting `List(T) -> [T; N]` would require the caller to supply `N`, which crosses into dependent type territory. Use `array_to_list` for one-way conversion.
...
| `map` | `(T -> U, [T; N]) -> Array(U, N)` | Transform array elements |
...
| `array_to_list` | `[T; N] -> List(T)` | Convert array to list |

`map` and `filter` for `[T; N]` are specialized per `N` at compile time. `list_to_array` is not provided — `N` is not tracked in `List(T)`, making the reverse conversion dependent on compile-time size information the list does not carry (see [§9.2](#92-standard-library-functions)).
