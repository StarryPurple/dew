...
def array_size = fn(x: T) -> Int {
  type_match typeof(x) {
    Array(_, N) => N,      // N bound to concrete size, usable as Int
    _           => -1,
  }
}
...
> **Not usable in function signatures.** `fn(x: Array(T, N)) -> N` would require dependent types (the return type depends on a value parameter). Const generic binding works only within `type_match` arm bodies where `N` is already resolved. Writing `-> N` in a function signature is rejected — use `-> Int` instead.
