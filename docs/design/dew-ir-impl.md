...
**Bool type**. `IrType::Bool` is lowered to LLVM `i1` at the register level. Bool-producing instructions (literal, comparison, And/Or/Not) emit `i1` values. Bool-consuming instructions (Br) expect `i1`. At struct/enum/tuple boundaries, `as_i64()` helper zexts `i1` → `i64` for storage in flat `{i64,...}` struct fields. Stdout also zexts `i1` → `i64` for printf. This mixed approach keeps Bool as `i1` in registers (matching LLVM's comparison semantics) while maintaining the `i64` struct/arena storage model.
...
**Multi-payload non-recursive enums**. Enums with multiple payload fields (e.g. `Shape { Circle(Int), Rect(Int, Int) }`) pack the payload into a nested LLVM struct `{ i64, { i64, i64 } }`. The `EnumProj` instruction extracts the payload struct (field 1) then extracts the individual field from within it. `EnumCons` builds a sub-struct for the payload fields, zero-filling unused slots, then inserts it at field 1. This two-level approach handles both single-field (Circle) and multi-field (Rect) variants in a single enum type.

**Rx test suite**. 50 `.rx` test files from the Rx→Dew interop pipeline are available under `RustShard-Compiler/test/`. These tests use boolean arrays up to `[bool; 10000]`. The typed array support in the LLVM backend allows these to be lowered as `[10000 x i1]` (≈1.2KB) rather than `[10000 x i64]` (80KB, exceeding the 64KB arena). The Rx translator is not yet integrated — these tests serve as target specifications for the interop pipeline.
